...
- Category-aware [bot identification and blocking](docs/BOT_MANAGEMENT.md) for 55 search, social, SEO, archive, and AI agents, with per-site/per-route policies, optional IP-range verification, per-bot graphs, and a Top bots view
- Opt-in [Tor and ASN network category detection](docs/NETWORK_PRIVACY.md) (VPN, datacenter, ISP/telecom, and more, discovered dynamically from a published category list), with monitor/block modes per site, route, or TCP/UDP stream, daily refreshes, and explicit IP/ASN allowlist precedence
...
BurrowGate's distinction is the integration: bot and privacy-network identity and blocking, protection decisions, browser challenges, local or OIDC-backed access accounts, signed upstream identity, bans, traffic records, and firewall updates share one policy and audit surface. Its bundled request-protection rules are intentionally small and do not inspect request bodies; they are not a replacement for the OWASP Core Rule Set. See [Bot identification and blocking](docs/BOT_MANAGEMENT.md), [Tor and ASN network category detection](docs/NETWORK_PRIVACY.md), [Single Sign-On](docs/SSO.md), [Cross-site authentication](docs/CROSS_SITE_AUTH.md), and [Managed request protection](docs/MANAGED_PROTECTION.md).
