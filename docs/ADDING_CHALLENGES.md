...
`src/challenges/providers/quiz.ts` is the one provider whose `config` is genuinely authored content rather than tuning knobs - a **Questions** list, each entry its own `{ question, correctAnswers[], wrongAnswers[] }` object, plus **Questions per round** and a **Pass threshold (%)**. The admin UI mirrors this: `public/admin.js`'s `qa-repeater` field type is a nested version of the same Add/Move up/Move down/Remove pattern the top-level challenge-step list already uses (`quizQuestionCardHtml`/`renumberQuizQuestions`/etc.), scoped to one step's config instead of a site's whole policy - the pattern to reach for if a future provider needs its own repeating sub-list. `create()` picks `questionCount` distinct questions at random (clamped down, not rejected, if the pool hasn't grown that large yet), and for each one, one random accepted phrasing from `correctAnswers` plus up to 3 random entries from _that question's own_ `wrongAnswers` - deliberately never a shared pool, since unrelated distractors (a "capital of France" question offering "Twelve" as an option) would make the quiz trivially guessable without even reading it. All answers are submitted together at the end (`{ choices: string[] }`) and `verify()` accepts the round once at least `ceil(passPercent/100 * questionCount)` of them match - rounded up, so a threshold can never be satisfied with less than that share. Like `snake`/`slider`/`trace`, a failed round collapses to one generic key (`wrongAnswer`), but unlike them it's genuinely weaker bot resistance: nothing here is a timing/motion signal or cryptographic proof, `verify()` is a plain string comparison, so it's meant as a novelty option, not a serious deterrent on its own.
