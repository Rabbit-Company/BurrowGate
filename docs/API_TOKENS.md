...
| `metric`         | `requests` or `uniqueIps`; applies to traffic, geography, paths, and referrers                                                                         | `requests` |
...
The version 1 response includes `schemaVersion`, `view`, `metric`, `title`, `scope`, `pathPrefix`, `path`, `successfulOnly`, `unit`, `hours`, `generatedAt`, `from`, `to`, `bucketSeconds`, `hasData`, `maximum`, `stats`, `rows`, and `points`. The echoed `metric` lets clients verify that the requested aggregation was applied. Dates use ISO 8601 UTC. Each stat has a `label`, numeric or null `value`, and `unit`. Each point has a Unix millisecond `timestamp`, numeric or null `value`, and a normalized chart `height` (0–100). Series contain at most 49 intervals. Geography returns one row per country with at least one request in the range, ordered by the selected metric, each with `label` (the country code, such as `DE`) and `value`. The list is complete rather than truncated, so clients decide how many rows to show. Unknown locations use `ZZ`. Private-network locations use `XX`. Clients map these codes to display names.

`paths` and `referrers` are the exception to that completeness. Both group by a value visitors control, so a single crawler can mint millions of distinct ones. Each returns at most the top 100 rows by the selected metric. Their stats describe those rows rather than the whole site. In request mode, `Requests in top paths` is the sum of the returned rows rather than the site total, so these views will disagree with `overview` whenever the cap is reached. Unique-IP row values are distinct within each individual page, country, or referrer and must not be added together because the same address can appear in several rows. Use `overview` for totals, and narrow with `pathPrefix` to see further down the tail of one subtree.

Bandwidth uses MiB per interval. Cache hit ratios and CPU use percentages. Request latency uses milliseconds. Memory and storage use GiB. Network charts show average download Mbps per interval, with average upload in the summary. Traffic summaries include the number of distinct client IP addresses in the requested site, path, success-status, and time scope. This deduplicates refreshes but is not an exact count of people because addresses can be shared or change. Request counts include all recorded outcomes. Blocked counts follow the dashboard's blocked-decision definition. Latency summaries are weighted by requests, using the dashboard's rounded interval averages.
