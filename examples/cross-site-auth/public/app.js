import { BrowserSessionAssertionClient, BURROWGATE_SESSION_ASSERTION_HEADER } from "/burrowgate-auth.js";
...
function authClient() {
	if (!assertions) throw new Error("Authentication client is still initializing");
	return assertions;
}
...
async function mintAssertion() {
	const assertion = await authClient().refresh();
	showOutput({ ...assertion, token: `${assertion.token.slice(0, 32)}...` });
	return assertion;
}

async function callApi(path = "/api/me") {
	const result = await readJsonResponse(await authClient().fetch(path));
...
logoutButton.addEventListener("click", async () => {
	try {
		previousAssertion = authClient().currentAssertion;
		...
		showOutput({ ok: true, previousAssertionPrefix: previousAssertion?.token.slice(0, 32) });
	} catch (error) {
		showStatus(error.message, "error");
		showOutput(error.body ?? String(error));
	}
...
revokedButton.addEventListener("click", async () => {
	...
	try {
		const result = await jsonRequest(`${demoConfig.backendOrigin}/api/me`, {
			headers: { [BURROWGATE_SESSION_ASSERTION_HEADER]: previousAssertion.token },
		});
...
try {
	...
	assertions = new BrowserSessionAssertionClient({
		apiBaseUrl: demoConfig.backendOrigin,
		onUpdate(assertion) {
			identity.textContent = `${assertion.user.username} (${assertion.user.id})`;
			showStatus(`Assertion ready until ${new Date(assertion.expiresAt).toLocaleTimeString()}; background refresh is active.`, "ok");
		},
		onError(error) {
			showStatus(`Background assertion refresh failed: ${error instanceof Error ? error.message : String(error)}`, "error");
		},
	});
	showStatus(`Frontend ${location.origin} will call ${demoConfig.backendOrigin}. Initializing the in-memory assertion manager.`);
} catch (error) {
	showStatus(error.message, "error");
	showOutput(error.body ?? String(error));
}
