...
export const repository = {
	...
	async updateStreamNetworkDefaults(
		streamId: string,
	...
	async insertEvent(event: RequestEventRecord): Promise<void> {
		await db`INSERT INTO request_events (id,site_id,session_id,ip,method,path,path_only,status,decision,latency_ms,country_code,asn,asn_org,origin_id,cache_status,protection_status,protection_rule_id,protection_category,protection_severity,protection_ruleset_id,protection_ruleset_version,protection_matches_json,access_username,origin_username,referer,referer_host,bot_id,bot_name,bot_category,bot_verified,network_privacy_json,crowdsec_json,request_body,request_body_truncated,request_content_type,request_headers,request_headers_truncated,response_headers,response_headers_truncated,created_at) VALUES (${event.id},${event.site_id},${event.session_id},${event.ip},${event.method},${event.path},${pathWithoutQuery(event.path)},${event.status},${event.decision},${event.latency_ms},${event.country_code},${event.asn},${event.asn_org},${event.origin_id ?? null},${event.cache_status},${event.protection_status},${event.protection_rule_id},${event.protection_category},${event.protection_severity},${event.protection_ruleset_id},${event.protection_ruleset_version},${event.protection_matches_json},${event.access_username ?? null},${event.origin_username ?? null},${event.referer ?? null},${event.referer_host ?? null},${event.bot_id ?? null},${event.bot_name ?? null},${event.bot_category ?? null},${event.bot_verified ?? null},${event.network_privacy_json ?? null},${event.crowdsec_json ?? null},${event.request_body ?? null},${event.request_body_truncated ?? null},${event.request_content_type ?? null},${event.request_headers ?? null},${event.request_headers_truncated ?? null},${event.response_headers ?? null},${event.response_headers_truncated ?? null},${event.created_at})`;
	},
	...
	async pagedEvents(query: EventQuery): Promise<PageResult<RequestEventRecord>> {
		...
		const searchFilter = pattern
			? db`AND (id=${exactSearch} OR LOWER(ip) LIKE ${pattern} OR LOWER(path) LIKE ${pattern} OR LOWER(COALESCE(protection_rule_id,'')) LIKE ${pattern} OR LOWER(COALESCE(access_username,'')) LIKE ${pattern} OR LOWER(COALESCE(origin_username,'')) LIKE ${pattern} OR LOWER(COALESCE(referer_host,'')) LIKE ${pattern} OR LOWER(COALESCE(bot_name,'')) LIKE ${pattern} OR session_id=${exactSearch})`
			: db``;
    ...
    `) as Array<{ count: number | string }>;
      ...
      SELECT id,site_id,session_id,ip,method,path,status,decision,latency_ms,country_code,asn,asn_org,origin_id,cache_status,
		...
		protection_matches_json,access_username,origin_username,referer,referer_host,bot_id,bot_name,bot_category,bot_verified,network_privacy_json,created_at,
