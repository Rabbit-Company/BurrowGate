...
function tabScopeFilter(scope: Exclude<TabMetricsScope, "bandwidth" | "sessions">) {
	switch (scope) {
		case "blocked":
			return db`AND decision IN ('blocked','route-blocked','bot-blocked')`;
		case "protection":
...
export const repository = {
	...
	async pagedEvents(query: EventQuery): Promise<PageResult<RequestEventRecord>> {
		...
		const searchFilter = pattern
			? db`AND (id=${exactSearch} OR LOWER(ip) LIKE ${pattern} OR LOWER(path) LIKE ${pattern} OR LOWER(COALESCE(protection_rule_id,'')) LIKE ${pattern} OR LOWER(COALESCE(access_username,'')) LIKE ${pattern} OR LOWER(COALESCE(referer_host,'')) LIKE ${pattern} OR LOWER(COALESCE(bot_name,'')) LIKE ${pattern} OR session_id=${exactSearch})`
			: db``;
    ...
    `) as Array<{ count: number | string }>;
      ...
      SELECT id,site_id,session_id,ip,method,path,status,decision,latency_ms,country_code,asn,asn_org,origin_id,cache_status,
		...
		protection_matches_json,access_username,referer,referer_host,bot_id,bot_name,bot_category,bot_verified,created_at,
	...
	async botMetrics(
		...
		bots: Array<{ id: string; key: string; name: string; category: string; verified: boolean; count: number; blocked: number }>;
		series: Array<{ bucket: number; [key: string]: number }>;
	}> {
		const siteFilter = siteScopeFilter(siteId);
		const totals = (await db`
			SELECT bot_id, MAX(bot_name) AS bot_name, MAX(bot_category) AS bot_category, MAX(COALESCE(bot_verified,0)) AS verified,
		...
		const rows = (await db`
			...
			FROM request_events
			WHERE created_at >= ${since} AND created_at <= ${until} AND bot_id IN ${db(ids)} ${siteFilter}
			GROUP BY ${bucket}, bot_id ORDER BY bucket ASC
		...
		for (const row of rows) {
			...
			if (point && key) point[key] = toNumber(row.count);
		}
		return { bots, series: points };
	},
	...
	async tabBotMetrics(
		...
		since: number,
		until: number,
	): Promise<Array<{ botId: string; name: string; category: string; verified: boolean; count: number }>> {
		if (Array.isArray(siteScope) && siteScope.length === 0) return [];
		const siteFilter = siteScopeFilter(siteScope);
		const rows = (await db`
