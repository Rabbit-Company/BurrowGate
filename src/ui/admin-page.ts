import { apiTokensCardMarkup, usersModalMarkup } from "./fragments/account-shared.ts";
import { APP_VERSION, authErrorToast, dashboardSwitchNav, escapeHtml, page, tablerIcon } from "./layout.ts";
...
<header class="row between responsive dashboard-header">
  <div><div class="brand"><span class="mark"></span> BurrowGate<span class="version-tag">v${escapeHtml(APP_VERSION)}</span></div>${dashboardSwitchNav("web-proxy")}<p id="siteDescription" class="muted header-subtitle">Reverse proxy control plane</p></div>
  ...
  <article class="card">
    ...
    <div class="toolbar traffic-toolbar">
      ...
      <label><span>Rows</span><select id="eventPageSize" class="select page-size"><option>25</option><option selected>50</option><option>100</option><option>200</option></select></label>
		...
		<div class="table-wrap"><table class="table"><thead><tr><th>${sortButton("Time", "created_at")}</th><th>${sortButton("IP", "ip")}</th><th data-column="traffic:site">Website</th><th data-column="traffic:country">${sortButton("Country", "country_code")}</th><th data-column="traffic:asn">${sortButton("ASN", "asn")}</th><th data-column="traffic:bot">Bot</th><th data-column="traffic:privacy">Network type</th><th data-column="traffic:method">${sortButton("Method", "method")}</th><th data-column="traffic:path">${sortButton("Path", "path")}</th><th data-column="traffic:referer">Referrer</th><th id="eventOriginColumnHeader" data-column="traffic:origin" class="hidden">Origin</th><th data-column="traffic:status">${sortButton("Status", "status")}</th><th data-column="traffic:decision">${sortButton("Decision", "decision")}</th><th data-column="traffic:cache">${sortButton("Cache", "cache_status")}</th><th data-column="traffic:protection">${sortButton("Protection", "protection_status")}</th><th data-column="traffic:latency">${sortButton("Latency", "latency_ms")}</th></tr></thead><tbody id="events"><tr><td colspan="13" class="empty-cell">Loading...</td></tr></tbody></table></div>
    ${pagination("events")}
  </article>
</section>
    ...
    <article class="card route-policy-editor-card">
      <div class="pad">
        ...
        <nav class="site-editor-tabs" aria-label="Route policy settings" role="tablist">
          ...
          <button class="site-editor-tab" role="tab" type="button" data-route-editor-tab="bots" aria-selected="false">Bots</button>
          <button class="site-editor-tab" role="tab" type="button" data-route-editor-tab="privacy" aria-selected="false">Privacy networks</button>
          <button class="site-editor-tab" role="tab" type="button" data-route-editor-tab="crowdsec" aria-selected="false">CrowdSec</button>
        ...
        <form id="routePolicyForm" class="site-form">
          ...
          <section class="error-response-editor hidden" data-route-editor-panel="bots">
            <div class="section-heading error-response-heading"><div><h3>Bot policy</h3><p class="muted">Bots are identified by User-Agent and optionally IP-verified. Override the site's blocked-bot list for this route.</p></div></div>
            <label><span>Route behavior</span><select id="routeBotPolicyMode" class="select"><option value="inherit">Inherit site bot policy</option><option value="override">Use a route-specific list</option></select><small class="muted">An override can block additional bots or allow bots blocked by the site.</small></label>
            <div id="routeBotPolicySettings" class="hidden"><p class="muted">Block an entire category or expand it to choose individual bots.</p><div id="routeBotPolicyList" class="json-field-grid bot-policy-grid"></div></div>
          </section>
          <section class="error-response-editor hidden" data-route-editor-panel="crowdsec">
            <div class="section-heading error-response-heading"><div><h3>CrowdSec</h3><p class="muted">How this route reacts to decisions from the CrowdSec Local API, and whether it forwards requests to the AppSec component.</p></div></div>
            <label><span>Route behavior</span><select id="routeCrowdSecPolicyMode" class="select"><option value="inherit">Inherit site CrowdSec policy</option><option value="override">Use route-specific modes</option></select><small class="muted">Explicit matching IP and ASN allow rules take precedence over CrowdSec.</small></label>
            <div id="routeCrowdSecSettings" class="site-form-grid hidden">
              <label><span>Ban decisions</span><select id="routeCrowdSecBan" class="select"><option value="disabled">Disabled</option><option value="monitor">Identify only</option><option value="block">Identify and block</option></select></label>
              <label><span>Captcha decisions</span><select id="routeCrowdSecCaptcha" class="select"><option value="disabled">Disabled</option><option value="monitor">Identify only</option><option value="challenge">Identify and challenge</option></select><small class="muted">Runs this route's challenge chain.</small></label>
              <label><span>AppSec inspection</span><select id="routeCrowdSecAppsec" class="select"><option value="disabled">Disabled</option><option value="monitor">Identify only</option><option value="block">Identify and block</option></select><small class="muted">Sends each request to the CrowdSec WAF. Adds a round-trip per request, so enable it on the routes that need it.</small></label>
            </div>
          </section>
          <section class="error-response-editor hidden" data-route-editor-panel="privacy">
            <div class="section-heading error-response-heading"><div><h3>Network privacy detection</h3><p class="muted">Identify or block Tor exits and ASN-classified network categories (VPN, datacenter, ISP, and more) for this route.</p></div></div>
            <label><span>Route behavior</span><select id="routeNetworkPrivacyPolicyMode" class="select"><option value="inherit">Inherit site privacy policy</option><option value="override">Use route-specific modes</option></select><small class="muted">Explicit matching IP and ASN allow rules take precedence over automatic category blocking.</small></label>
            <div id="routeNetworkPrivacySettings" class="site-form-grid hidden"></div>
            <p class="notice muted">ASN category identification requires GeoLite2 ASN enrichment and is best-effort classification. Shared networks may carry legitimate traffic. Use Identify only before enabling blocking.</p>
          </section>
          ...
          <section class="error-response-editor hidden" data-route-editor-panel="http">
            ...
            <div class="section-heading error-response-heading"><div><h3>CORS</h3><p class="muted">Override the site's CORS policy for requests matching this route.</p></div></div>
            <div class="site-form-grid">
              <label><span>CORS mode</span><select id="routeHttpCorsMode" class="select"><option value="inherit">Inherit site default</option><option value="enabled">Enable</option><option value="disabled">Disable</option></select><small class="muted">The route override applies only when this policy matches.</small></label>
              <label><span>Preflight cache (seconds)</span><input id="routeHttpCorsMaxAge" class="input" type="number" min="0" max="86400" step="1" placeholder="Inherit site"></label>
              <label><span>Allow credentials</span><select id="routeHttpCorsCredentials" class="select"><option value="">Inherit site</option><option value="true">Allow</option><option value="false">Deny</option></select></label>
              <label class="site-origin-field"><span>Allowed origins</span><textarea id="routeHttpCorsOrigins" class="input code-input compact-code-input" rows="3" spellcheck="false" placeholder="Inherit site origins"></textarea><small class="muted">Comma or whitespace separated. Blank inherits.</small></label>
              <label class="site-origin-field"><span>Allowed methods</span><textarea id="routeHttpCorsMethods" class="input code-input compact-code-input" rows="2" spellcheck="false" placeholder="Inherit site methods"></textarea><small class="muted">Comma or whitespace separated. Blank inherits.</small></label>
              <label class="site-origin-field"><span>Allowed request headers</span><textarea id="routeHttpCorsHeaders" class="input code-input compact-code-input" rows="2" spellcheck="false" placeholder="Inherit site headers"></textarea><small class="muted">Comma or whitespace separated. Blank inherits.</small></label>
              <label class="site-origin-field"><span>Exposed response headers</span><textarea id="routeHttpCorsExposedHeaders" class="input code-input compact-code-input" rows="2" spellcheck="false" placeholder="Inherit site headers"></textarea><small class="muted">Comma or whitespace separated. Blank inherits.</small></label>
            </div>
          </section>
    ...
    <article class="card site-editor-card">
      <div class="pad">
        ...
        <nav class="site-editor-tabs" aria-label="Site settings" role="tablist">
          ...
          <button class="site-editor-tab" role="tab" type="button" data-site-editor-tab="bots" aria-selected="false">Bots</button>
          <button class="site-editor-tab" role="tab" type="button" data-site-editor-tab="privacy" aria-selected="false">Privacy networks</button>
          <button class="site-editor-tab" role="tab" type="button" data-site-editor-tab="crowdsec" aria-selected="false">CrowdSec</button>
        ...
        <form id="siteForm" class="site-form">
          ...
          <section class="error-response-editor hidden" data-site-editor-panel="bots">
            <div class="section-heading error-response-heading"><div><h3>Bot identification and blocking</h3><p class="muted">Bots are identified from the User-Agent header and counted automatically. Select bots to reject with HTTP 403 before they reach the origin.</p></div></div>
            <p class="notice muted">For bots with a published IP feed, BurrowGate checks only that bot's ranges and marks matching requests as IP verified. User-Agent identities can be spoofed, and blocking applies whether or not the address is verified.</p>
            <p class="muted">Block an entire category or expand it to choose individual bots.</p>
            <div id="siteBotPolicyList" class="json-field-grid bot-policy-grid"></div>
          </section>
          <section class="error-response-editor hidden" data-site-editor-panel="crowdsec">
            <div class="section-heading error-response-heading"><div><h3>CrowdSec</h3><p class="muted">How this site reacts to decisions pulled from the CrowdSec Local API. Connect one from the CrowdSec tab first. Sites start in Identify only, so nothing is blocked until you change it.</p></div></div>
            <div class="site-form-grid">
              <label><span>Ban decisions</span><select id="siteCrowdSecBan" class="select"><option value="disabled">Disabled</option><option value="monitor">Identify only</option><option value="block">Identify and block</option></select></label>
              <label><span>Captcha decisions</span><select id="siteCrowdSecCaptcha" class="select"><option value="disabled">Disabled</option><option value="monitor">Identify only</option><option value="challenge">Identify and challenge</option></select><small class="muted">Runs the site's challenge chain instead of refusing the request.</small></label>
              <label><span>AppSec inspection</span><select id="siteCrowdSecAppsec" class="select"><option value="disabled">Disabled</option><option value="monitor">Identify only</option><option value="block">Identify and block</option></select><small class="muted">Sends each request to the CrowdSec WAF for inspection. This costs an HTTP round-trip per request, so it is off by default.</small></label>
            </div>
          </section>
          <section class="error-response-editor hidden" data-site-editor-panel="privacy">
            <div class="section-heading error-response-heading"><div><h3>Network privacy detection</h3><p class="muted">Classify Tor exits and best-effort ASN network categories (VPN, datacenter, ISP, and more) from public network data. Every category is disabled by default.</p></div></div>
            <div id="siteNetworkPrivacySettings" class="site-form-grid"></div>
            <p class="notice muted">ASN categories require GeoLite2 ASN enrichment and are best-effort classification. Explicit matching IP and ASN rules set to Allow take precedence over automatic category blocking. Start with Identify only and review Recent traffic before blocking broad ASN categories.</p>
          </section>
          ...
          <section class="error-response-editor hidden" data-site-editor-panel="http">
            ...
            <div class="site-form-grid">
              ...
              <label><span>Set response headers</span><textarea id="siteHttpResponseHeadersSet" class="input code-input" rows="6" spellcheck="false" placeholder="Cache-Control: no-store"></textarea><small class="muted">Applied to responses returned by the origin.</small></label>
            ...
            <div class="section-heading error-response-heading"><div><h3>CORS</h3><p class="muted">Answer cross-origin browser preflight requests directly and add the matching Access-Control headers to real responses. Disabled by default.</p></div></div>
            <label class="check-row"><input id="siteHttpCorsEnabled" type="checkbox"><span><strong>Enable CORS</strong><small class="muted">Preflight (OPTIONS) requests are answered directly and skip verification and access-list sign-in.</small></span></label>
            <div id="siteHttpCorsSettings" class="site-form-grid hidden">
              <label class="site-origin-field"><span>Allowed origins</span><textarea id="siteHttpCorsOrigins" class="input code-input compact-code-input" rows="3" spellcheck="false" placeholder="https://app.example.com"></textarea><small class="muted">Comma or whitespace separated. Use <code>*</code> to allow any origin - not allowed together with credentials.</small></label>
              <label class="site-origin-field"><span>Allowed methods</span><textarea id="siteHttpCorsMethods" class="input code-input compact-code-input" rows="2" spellcheck="false"></textarea><small class="muted">Comma or whitespace separated, e.g. <code>GET, POST</code>.</small></label>
              <label class="site-origin-field"><span>Allowed request headers</span><textarea id="siteHttpCorsHeaders" class="input code-input compact-code-input" rows="2" spellcheck="false"></textarea><small class="muted">Headers the browser may send, e.g. <code>content-type, authorization</code>.</small></label>
              <label class="site-origin-field"><span>Exposed response headers</span><textarea id="siteHttpCorsExposedHeaders" class="input code-input compact-code-input" rows="2" spellcheck="false"></textarea><small class="muted">Response headers readable by browser JavaScript. Empty by default.</small></label>
              <label class="check-row"><input id="siteHttpCorsCredentials" type="checkbox"><span><strong>Allow credentials</strong><small class="muted">Sends Access-Control-Allow-Credentials. Cannot be combined with a <code>*</code> allowed origin.</small></span></label>
              <label><span>Preflight cache (seconds)</span><input id="siteHttpCorsMaxAge" class="input" type="number" min="0" max="86400" step="1" required><small class="muted">How long browsers may cache a preflight response.</small></label>
            </div>
            <p id="siteHttpCorsCredentialsWarning" class="notice muted hidden">Allowed origins cannot include <code>*</code> while credentials are allowed - narrow the allowed origins to specific values.</p>
            <div class="section-heading error-response-heading"><div><h3>HSTS</h3><p class="muted">Tell browsers to only reach this site over HTTPS. Disabled by default.</p></div></div>
            <label class="check-row"><input id="siteHttpHstsEnabled" type="checkbox"><span><strong>Enable HSTS</strong><small class="muted">Sends Strict-Transport-Security on every HTTPS response for this site.</small></span></label>
            <div id="siteHttpHstsSettings" class="site-form-grid hidden">
              <label><span>Max age (seconds)</span><input id="siteHttpHstsMaxAge" class="input" type="number" min="0" max="63072000" step="1" required><small id="siteHttpHstsMaxAgeDays" class="muted"></small></label>
              <label class="check-row"><input id="siteHttpHstsIncludeSubDomains" type="checkbox"><span><strong>Include subdomains</strong><small class="muted">Applies the policy to every subdomain of this site's hostname too.</small></span></label>
              <label class="check-row"><input id="siteHttpHstsPreload" type="checkbox"><span><strong>Preload</strong><small class="muted">Requires include subdomains and a max age of at least 1 year. Submitting to a browser preload list is very hard to reverse - only enable this once you're certain every subdomain supports HTTPS.</small></span></label>
            </div>
          </section>
          <section class="error-response-editor hidden" data-site-editor-panel="access">
