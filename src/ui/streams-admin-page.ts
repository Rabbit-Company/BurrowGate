...
<header class="row between responsive dashboard-header">
  <div><div class="brand"><span class="mark"></span> BurrowGate<span class="version-tag">v${escapeHtml(APP_VERSION)}</span></div>${dashboardSwitchNav("streams")}<p class="muted header-subtitle">TCP and UDP stream proxy control plane</p></div>
...
<section class="card geo-card">
  <div class="pad row between responsive"><div><h2>Geographic distribution</h2><p id="geoSubtitle" class="muted">Active connections by country</p></div><select id="geoMetricMode" class="select select-small"><option value="active">Active connections</option><option value="events">Traffic log</option><option value="bandwidth">Bandwidth</option><option value="blocked">Blocked connections</option></select></div>
...
<section id="streamAsnCard" class="card pad">
  <div class="row between responsive">
    <div><h2>Top ASNs</h2><p class="muted">Network providers behind active connections</p></div>
    <strong id="streamAsnTotal">0</strong>
  </div>
  <div id="streamAsnList" class="geo-country-list"><p class="muted">No ASN data is available.</p></div>
</section>

<section id="streamRefererCard" class="card pad">
  <div class="row between responsive">
    <div><h2 id="streamRefererTitle">Top blocked IPs</h2><p id="streamRefererSubtitle" class="muted">Source IPs blocked most often in the selected range</p></div>
    <div class="row"><select id="streamTopListMode" class="select select-small"><option value="blocked">Top blocked IPs</option><option value="bandwidth">Top IPs by bandwidth</option><option value="errors">Top error reasons</option></select><strong id="streamRefererTotal">0</strong></div>
  </div>
...
<section id="panel-connections" class="tab-panel"><article class="card"><div class="pad section-heading"><div><h2>Live TCP connections and UDP peers</h2><p id="udpPeerNotice" class="muted">UDP peers disconnect after the configured idle timeout.</p></div><div id="connectionsHeaderActions" class="row"><button id="refreshConnections" class="button secondary" type="button">Refresh</button></div></div><div class="toolbar"><label class="search-field"><span>Search</span><input id="connectionsSearch" class="input" placeholder="IP, username..."></label><label><span>Protocol</span><select id="connectionsProtocol" class="select"><option value="">All</option><option value="tcp">TCP</option><option value="udp">UDP</option></select></label><label><span>Country</span><select id="connectionsCountry" class="select"><option value="">All countries</option></select></label><label><span>ASN</span><input id="connectionsAsn" class="input" type="number" min="1" step="1" placeholder="e.g. 15169"></label><label><span>Rows</span><select id="connectionsPageSize" class="select page-size"><option>25</option><option selected>50</option><option>100</option><option>200</option></select></label></div><div class="table-wrap"><table class="table"><thead><tr><th>${sortButton("Protocol", "protocol")}</th><th>${sortButton("Incoming port", "incomingPort")}</th><th>${sortButton("Client", "clientIp")}</th><th data-column="connections:stream">Stream</th><th data-column="connections:country">${sortButton("Country", "countryCode")}</th><th data-column="connections:asn">${sortButton("ASN", "asn")}</th><th data-column="connections:privacy">Network type</th><th data-column="connections:connected">${sortButton("Connected", "connectedAt")}</th><th data-column="connections:lastActivity">${sortButton("Last activity", "lastActivityAt")}</th><th data-column="connections:toOrigin">${sortButton("To origin", "clientToUpstreamBytes")}</th><th data-column="connections:toClient">${sortButton("To client", "upstreamToClientBytes")}</th></tr></thead><tbody id="activeConnections"><tr><td colspan="9" class="empty-cell">Loading...</td></tr></tbody></table></div>${pagination("connections")}</article></section>

<section id="panel-events" class="tab-panel hidden"><article class="card"><div class="pad section-heading"><div><h2>Connection log</h2><p class="muted">TCP socket lifecycle events and synthetic UDP peer sessions.</p></div><div id="eventsHeaderActions" class="row"><button id="refreshEvents" class="button secondary" type="button">Refresh</button></div></div><div class="toolbar"><label class="search-field"><span>Search</span><input id="eventSearch" class="input" placeholder="IP, connection, reason, rule ID..."></label><label id="eventProtocolFilter"><span>Protocol</span><select id="eventProtocol" class="select"><option value="">All</option><option value="tcp">TCP</option><option value="udp">UDP</option></select></label><label><span>Event</span><select id="eventType" class="select"><option value="">All</option><option value="connected">Connected</option><option value="disconnected">Disconnected</option><option value="monitored">Monitored</option><option value="blocked">Blocked</option><option value="throttled">Throttled</option><option value="upstream-error">Upstream error</option><option value="listener-error">Listener error</option></select></label><label><span>Country</span><select id="eventCountry" class="select"><option value="">All countries</option></select></label><label><span>ASN</span><input id="eventAsn" class="input" type="number" min="1" step="1" placeholder="e.g. 15169"></label></div><div class="table-wrap"><table class="table"><thead><tr><th>${sortButton("Time", "created_at")}</th><th>${sortButton("Client", "client_ip")}</th><th data-column="events:stream">Stream</th><th data-column="events:country">${sortButton("Country", "country_code")}</th><th data-column="events:asn">${sortButton("ASN", "asn")}</th><th data-column="events:privacy">Network type</th><th data-column="events:event">${sortButton("Event", "event_type")}</th><th class="protocol-column">${sortButton("Protocol", "protocol")}</th><th class="port-column">${sortButton("Port", "incoming_port")}</th><th data-column="events:reason">${sortButton("Reason", "reason")}</th><th data-column="events:rule">${sortButton("Rule", "protection_rule_id")}</th><th data-column="events:toOrigin">${sortButton("To origin", "client_to_upstream_bytes")}</th><th data-column="events:toClient">${sortButton("To client", "upstream_to_client_bytes")}</th><th data-column="events:connectionId">${sortButton("Connection ID", "connection_id")}</th></tr></thead><tbody id="streamEvents"><tr><td colspan="12" class="empty-cell">Open the tab to load events.</td></tr></tbody></table></div>${pagination("events")}</article></section>
...
<section id="panel-rules" class="tab-panel hidden">
  ...
  <article class="card network-defaults-card">
    <div class="pad section-heading"><div><h2>Default network actions</h2><p class="muted">Explicit IP rules override ASN rules, which override country rules, which override these defaults.</p></div><button id="saveStreamNetworkDefaults" class="button" type="button">Save</button></div>
  ...
  <article class="card country-rule-card">
    <div class="pad"><h2>Add an ASN rule</h2><form id="streamAsnRuleForm" class="country-rule-form"><label><span>ASN</span><input id="streamAsnRuleAsn" class="input" type="number" min="1" step="1" name="asn" placeholder="e.g. 15169" required></label><label><span>Action</span><select class="select" name="action"><option value="block">Block</option><option value="allow">Allow</option></select></label><label><span>Expires</span><input class="input" type="datetime-local" name="expiresAt"></label><label class="reason-field"><span>Reason</span><input class="input" name="reason" placeholder="Optional reason"></label><button class="button align-end" type="submit">Add rule</button></form></div>
    <div class="table-wrap"><table class="table"><thead><tr><th>State</th><th>ASN</th><th>Action</th><th>Reason</th><th>Created</th><th>Expires</th><th></th></tr></thead><tbody id="streamAsnRules"><tr><td colspan="7" class="empty-cell">Open the tab to load ASN rules.</td></tr></tbody></table></div>
  </article>
</section>
...
<section id="panel-protection" class="tab-panel hidden">
  ...
  <article class="card">
    <div class="pad section-heading"><div><h2>Bandwidth limit</h2><p class="muted">Temp-bans an IP that pushes more than the configured amount of bandwidth through this stream's TCP or UDP side within the time window. Detection is periodic rather than instantaneous to keep the proxy path fast.</p></div><button id="saveStreamBandwidthLimit" class="button" type="button">Save</button></div>
    <div class="pad pad-topless">
      <div class="section-heading error-response-heading"><div><h3>TCP</h3></div></div>
      <div class="site-form-grid">
        <label class="check-row compact-check"><input id="streamBandwidthLimitTcpEnabled" type="checkbox"><span><strong>Enabled</strong><small class="muted">Off by default.</small></span></label>
        <label><span>Threshold (MiB)</span><input id="streamBandwidthLimitTcpMaxMiB" class="input" type="number" min="1" step="1" value="50" required></label>
        <label><span>Window (seconds)</span><input id="streamBandwidthLimitTcpWindowSeconds" class="input" type="number" min="1" max="3600" step="1" value="60" required></label>
        <label><span>Ban duration (seconds)</span><input id="streamBandwidthLimitTcpBanSeconds" class="input" type="number" min="0" max="2592000" step="1" value="3600" required></label>
      </div>
      <div class="section-heading error-response-heading"><div><h3>UDP</h3></div></div>
      <div class="site-form-grid">
        <label class="check-row compact-check"><input id="streamBandwidthLimitUdpEnabled" type="checkbox"><span><strong>Enabled</strong><small class="muted">Off by default.</small></span></label>
        <label><span>Threshold (MiB)</span><input id="streamBandwidthLimitUdpMaxMiB" class="input" type="number" min="1" step="1" value="50" required></label>
        <label><span>Window (seconds)</span><input id="streamBandwidthLimitUdpWindowSeconds" class="input" type="number" min="1" max="3600" step="1" value="60" required></label>
...
<section id="panel-health" class="tab-panel hidden">
  ...
  <article class="card">
    ...
    <div id="streamHealthSettings" class="pad pad-topless site-form-grid">
      ...
      <label><span>Check interval (seconds)</span><input id="streamHealthInterval" class="input" type="number" min="3" max="3600" value="10" required></label>
