...
import {
	HA_AUTO_FAILOVER_ENABLED,
	HA_AUTO_FAILOVER_MIN_MEMBERS,
	HA_ELECTION_TIMEOUT_BASE_MS,
	HA_ELECTION_TIMEOUT_JITTER_MS,
	HA_QUORUM_FENCE_SCHEDULING_MARGIN_MS,
	HA_QUORUM_LOSS_FENCE_MS,
	HA_REPLICA_LIVENESS_TIMEOUT_MS,
} from "./ha-timing.ts";
import { isSecureHaUrl } from "./ha-url.ts";
import { parseTrustedProxyCidrs } from "./services/incoming-proxy-protocol.ts";
...
export type HaRole = "primary" | "replica";

function haRole(value: string | undefined): HaRole {
	if (!value || !value.trim()) return "primary";
	const role = value.trim().toLowerCase();
	if (role === "primary" || role === "replica") return role;
	throw new Error("BG_HA_ROLE must be primary or replica");
}
...
export const config = {
	...
	http: {
		...
		proxyProtocol: envBoolean("BG_HTTP_PROXY_PROTOCOL", false),
	},
	https: {
		...
		proxyProtocol: envBoolean("BG_HTTPS_PROXY_PROTOCOL", false),
		listenerDrainTimeoutMs: envNumber("BG_TLS_LISTENER_DRAIN_TIMEOUT_MS", 5_000, 100, 60_000),
		http2Enabled: envBoolean("BG_HTTP2_ENABLED", false),
		http3Enabled: envBoolean("BG_HTTP3_ENABLED", false),
	},
	proxyProtocol: {
		allowDirect: envBoolean("BG_PROXY_PROTOCOL_ALLOW_DIRECT", true),
		trustedCidrs: parseTrustedProxyCidrs((process.env.BG_PROXY_PROTOCOL_TRUSTED_CIDRS ?? "").split(/[\s,]+/u).filter(Boolean)),
	},
	...
	firewallSync: {
		...
		requestTimeoutMs: envNumber("BG_FIREWALL_SYNC_REQUEST_TIMEOUT_MS", 10_000, 1_000, 60_000),
		nftTimeoutMs: envNumber("BG_FIREWALL_SYNC_NFT_TIMEOUT_MS", 5_000, 500, 60_000),
	...
	ha: {
		...
		port: envNumber("BG_HA_PORT", 7443, 1, 65535),
		primaryUrl: process.env.BG_HA_PRIMARY_URL?.trim() || (null as string | null),
		primaryAdminUrl: process.env.BG_HA_PRIMARY_ADMIN_URL?.trim() || (null as string | null),
		sharedToken: process.env.BG_HA_SHARED_TOKEN?.trim() || (null as string | null),
		reconnectMinDelayMs: envNumber("BG_HA_RECONNECT_MIN_DELAY_MS", 1_000, 100, 60_000),
		reconnectMaxDelayMs: envNumber("BG_HA_RECONNECT_MAX_DELAY_MS", 30_000, 1_000, 300_000),
		disconnectedReadyGraceSeconds: envNumber("BG_HA_DISCONNECTED_READY_GRACE_SECONDS", 60, 0, 86_400),
		maxSyncStalenessSeconds: envNumber("BG_HA_MAX_SYNC_STALENESS_SECONDS", 30, 5, 86_400),
		changelogPageSize: envNumber("BG_HA_CHANGELOG_PAGE_SIZE", 500, 10, 5_000),
		changelogRetentionDays: envNumber("BG_HA_CHANGELOG_RETENTION_DAYS", 30, 1, 3_650),
		tlsCertFile: process.env.BG_HA_TLS_CERT_FILE?.trim() || (null as string | null),
		tlsKeyFile: process.env.BG_HA_TLS_KEY_FILE?.trim() || (null as string | null),
		caFile: process.env.BG_HA_CA_FILE?.trim() || (null as string | null),
		nodeName: process.env.BG_HA_NODE_NAME?.trim() || hostname(),
		selfAdminUrl: null as string | null,
		fencedForPromotion: false,
		authorityFence: null as null | { observedEpoch: number; sourceNodeId: string; observedAt: number },
		versionMismatchNodes: [] as Array<{ nodeId: string; name: string; version: string }>,
		epoch: 0,
		autoFailoverEnabled: HA_AUTO_FAILOVER_ENABLED,
		autoFailoverMinMembers: HA_AUTO_FAILOVER_MIN_MEMBERS,
		electionTimeoutBaseSeconds: HA_ELECTION_TIMEOUT_BASE_MS / 1_000,
		electionTimeoutJitterMs: HA_ELECTION_TIMEOUT_JITTER_MS,
		quorumLossFenceSeconds: HA_QUORUM_LOSS_FENCE_MS / 1_000,
		quorumFenced: false,
		lastMajorityConfirmedAt: Date.now(),
		quorumTrackingActive: false,
...
export function validateHaConfig(ha: typeof config.ha): void {
	if (!ha.sharedToken) throw new Error("BG_HA_SHARED_TOKEN must be set");
	if (ha.role === "replica" && (!ha.primaryUrl || !ha.primaryAdminUrl)) {
		throw new Error("BG_HA_PRIMARY_URL and BG_HA_PRIMARY_ADMIN_URL must be set when BG_HA_ROLE is replica");
	}
	if (ha.primaryUrl && !isSecureHaUrl(ha.primaryUrl)) throw new Error("BG_HA_PRIMARY_URL must be a valid absolute HTTPS URL without embedded credentials");
	if (ha.primaryAdminUrl && !isSecureHaUrl(ha.primaryAdminUrl))
		throw new Error("BG_HA_PRIMARY_ADMIN_URL must be a valid absolute HTTPS URL without embedded credentials");
	if (ha.selfAdminUrl && !isSecureHaUrl(ha.selfAdminUrl))
		throw new Error("This node's HA admin URL must be a valid absolute HTTPS URL without embedded credentials");

	if (
		ha.autoFailoverEnabled &&
		HA_REPLICA_LIVENESS_TIMEOUT_MS + ha.quorumLossFenceSeconds * 1_000 + HA_QUORUM_FENCE_SCHEDULING_MARGIN_MS >= ha.electionTimeoutBaseSeconds * 1_000
	) {
		throw new Error(
			"Unsafe HA failover timing constants: HA_QUORUM_LOSS_FENCE_MS plus replica-liveness detection must be shorter than HA_ELECTION_TIMEOUT_BASE_MS so an old primary fences before replicas can elect a replacement",
		);
	}
}
